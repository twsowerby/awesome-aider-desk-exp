 ...
 * Tilly uses content-production conventions.
 * Valid types: draft, edit, refine, research, revise, proofread
 */
export function generateFallbackMessage(agentRole: string, taskDescription: string, type = 'draft'): string {
  const validTypes = ['draft', 'edit', 'refine', 'research', 'revise', 'proofread'];
  const contentPrefix = validTypes.includes(type) ? type : 'draft';
  return `${contentPrefix}: ${taskDescription} (by ${agentRole})`;
}
